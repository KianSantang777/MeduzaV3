...
F=("__init__.bin","inis.bin","it_.bin","core.bin")

def self_check(meta):
 try:
  p=os.path.abspath(__file__)
  with open(p,"rb") as f:d=f.read()
  if hashlib.sha256(d).hexdigest()!=meta["runner"]:
   raise SystemExit
 except Exception:
  raise SystemExit
...
def m():
 try:
  ...
  if hashlib.sha1(raw).hexdigest()!=meta["s1"]:
   raise SystemExit

  if hashlib.sha384(raw).hexdigest()!=meta["s384"]:
   raise SystemExit
...
def r():
 ...
 try:
  ...
  try:
   ...
   src=d.decode("latin-1")

  exec(compile(src,os.path.abspath(__file__),"exec"),{"__name__":"__main__","__file__":os.path.abspath(__file__),"sys":sys})
