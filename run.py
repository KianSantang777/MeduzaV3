...
def is_termux():
    return (
        ...
        or (
            ...
            and "com.termux" in os.environ.get("PREFIX", "").lower()
        )
    )
def get_os_name():
    if is_termux():
...
def load_and_run(binary_path):
    ...
    try:
        ...
        print(f"[ERROR] Failed to load or execute module: {exc}")
        sys.exit(1)
def main():
    print("=" * 50)
    print(f"  {MODULE_NAME}")
    ...
    print(f"Expected module: {MODULE_NAME}{extension}")
    print()
    print("SOLUTION:")
    if is_termux():
        print("  Compile the module for Android/Termux.")
        print("  The resulting file must be a compatible .so binary.")
    else:
        print(f"  Ask the developer to compile for {current_os}.")
